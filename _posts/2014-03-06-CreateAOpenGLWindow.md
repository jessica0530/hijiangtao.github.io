...
在老师的推荐下准备系统学习下Nehe关于OpenGL的教程，在打开chm的第一眼就被作者的开源奉献精神所深深打动，这种思想与我不谋而合。

以下内容为我学习了教程之后自己的操作与总结，由于硬件与软件环境的差异，我的笔记内容在代码和描述上与原文可能存在一定出路。

----

第一课是教我们如何建立一个OpenGL空窗口，并实现它的全屏与窗口切换功能。

首先需要包括的是库文件的头文件与程序中所需要用到的变量声明：
...
对OpenGL进行所有的设置以及所有的绘图的代码如下，其实这一块有些代码我也是敲的半知半解。
...
接下来是程序退出时调用的代码，作者在这上面做了很多功夫，加了很多错误检测的信息，以下是作者在书中的描述以及代码部分：
...
在创建窗口部分，函数包括五个参数：窗口的标题栏，窗口的宽度，窗口的高度，色彩位数(16/24/32)，和全屏标志(TRUE --全屏模式， FALSE--窗口模式 )。返回的布尔值告诉我们窗口是否成功创建。
...
CS_HREDRAW 和 CS_VREDRAW 的意思是无论何时，只要窗口发生变化时就强制重画。CS_OWNDC为窗口创建一个私有的DC。这意味着DC不能在程序间共享。WndProc是我们程序的消息处理过程。由于没有使用额外的窗口数据，后两个字段设为零。然后设置实例。
